...
export default function Women() {
  const products = (productsData as Product[]).filter((product) => product.gender === 'women');
  const categories = (categoriesData as Category[]).filter((category) => category.gender === 'women');
  ...
  const applyQuickPreset = (preset: 'all' | 'best' | 'new' | 'dresses' | 'under-25') => {
    if (preset === 'all') {
      clearAllFilters();
      return;
    }
    ...
    if (preset === 'dresses') {
      setSelectedCategories(['women-dresses']);
      setSortBy('default');
      setPriceRange([0, MAX_PRICE]);
      return;
    }
        ...
        <div className="mt-4 space-y-3">
          {categories.map((category) => (
            <div key={category.id} className="flex items-center gap-2.5">
              <Checkbox
                id={`women-${category.id}`}
                checked={selectedCategories.includes(category.id)}
                onCheckedChange={() => toggleCategory(category.id)}
              />
              <Label htmlFor={`women-${category.id}`} className="cursor-pointer text-sm">
                {category.name}
              </Label>
            </div>
          ))}
  ...
  return (
    <div className="min-h-screen bg-background">
      <section className="relative overflow-hidden">
        <img src={heroWomen} alt="Women's collection" className="h-[270px] w-full object-cover object-top md:h-[320px]" />
        ...
        <div className="absolute inset-0 mx-auto flex max-w-6xl items-end px-4 pb-8 sm:px-6 lg:px-8">
          <div className="max-w-lg rounded-2xl border border-white/20 bg-black/35 p-4 backdrop-blur-[2px] sm:p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-primary-foreground/75">Women</p>
            <h1 className="mt-2 text-4xl font-bold tracking-tight text-primary-foreground md:text-5xl">Clean Fits For Real Days</h1>
            <p className="mt-2 text-sm text-primary-foreground/85 md:text-base">
              Dresses, tops, trousers, and accessories you can actually style every week.
            </p>
          </div>
        </div>
      </section>
        ...
        <section className="rounded-2xl border border-border/80 bg-card p-4 sm:p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button size="sm" variant="outline" className="rounded-full px-3 text-xs" onClick={() => applyQuickPreset('dresses')}>Dresses</Button>
          ...
          <section className="lg:hidden">
            <Sheet>
              ...
              <SheetContent side="left" className="w-[90vw] max-w-[340px] overflow-y-auto">
                <SheetHeader>
                  <SheetTitle>Filter Women's Shop</SheetTitle>
                </SheetHeader>
                <div className="mt-5">
                  <FilterContent />
                </div>
